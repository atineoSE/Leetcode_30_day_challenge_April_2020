...
open class XCTestCase: NSObject {
    ...
    open func tearDown() {
        ...
        guard let startTime = startTime, let endTime = endTime else {
            print ("\(testName): no timing info")
            return
        }
        let diff = endTime - startTime
        let diffAsString = NSString.init(format: "%.2f", diff)
        print("\(testName): \(diffAsString) seconds")
    }
    ...
    private func runTestMethods() {
        ...
        (0 ..< mc).forEach { _ in
            ...
            testName = String(describing: m)
            if let testName = testName, testName.hasPrefix("test") {
                self.setUp()
