...
class Solution {
    ...
    func minPathSumDynamicProgramming(_ grid: [[Int]]) -> Int {
        func getMinimumDistance(_ i: Int, _ j: Int) -> Int? {
            guard i >= 0 else { return nil }
            guard j >= 0 else { return nil }
            return minimumDistanceMatrix[i][j]
        }

        var minimumDistanceMatrix: [[Int]] = Array.init(repeating: Array.init(repeating: 0, count: m), count: n)
        var i = 0
        while i < n {
            var j = 0
            while j < m {
                let left = getMinimumDistance(i, j-1)
                let up = getMinimumDistance(i-1, j)
                
                if let left = left, let up = up {
                    minimumDistanceMatrix[i][j] = Swift.min(left, up) + grid[i][j]
                } else if let left = left {
                    minimumDistanceMatrix[i][j] = left + grid[i][j]
                } else if let up = up {
                    minimumDistanceMatrix[i][j] = up + grid[i][j]
                } else {
                    minimumDistanceMatrix[i][j] = grid[i][j]
                }
        ...
        return minimumDistanceMatrix[n-1][m-1]
    }
